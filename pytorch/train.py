...
parser = argparse.ArgumentParser(description='PyTorch TransVW Training')
...
training_data = TransVW_Dataset(x_train, y_train, conf)
validation_data = TransVW_Dataset(x_valid, y_valid, conf)
...
if os.path.exists(os.path.join(conf.model_path,"TransVW_chest_ct.pt")):
	checkpoint=torch.load(os.path.join(conf.model_path,"TransVW_chest_ct.pt"))
	model.load_state_dict(checkpoint['state_dict'])
	optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
	intial_epoch=checkpoint['epoch']
	best_loss = checkpoint['best_loss']
	num_epoch_no_improvement = checkpoint['num_epoch_no_improvement']
	print("Loading weights from ",os.path.join(conf.model_path,"TransVW_chest_ct.pt"),file=conf.log_writter)
...
for epoch in range(intial_epoch,conf.nb_epoch):
	...
	if  valid_loss < best_loss:
		...
		torch.save({
			...
			'best_loss': best_loss,
			'best_acc': best_acc
		}, os.path.join(conf.model_path, "TransVW_chest_ct.pt"))
		print("Saving model ", os.path.join(conf.model_path, "TransVW_chest_ct.pt"),
