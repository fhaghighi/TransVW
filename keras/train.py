...
parser.add_option("--nb_val_instances", dest="nb_val_instances", help="nubmer of validation instances in each class",type=int, default=30)
...
parser.add_option("--data_dir", dest="data_dir",help="data path", default=None)
...
config = setup_config(decoder_block_type=options.decoder_block_type,
                      ...
                      nb_val_instances = options.nb_val_instances,
...
for i in range (int(math.ceil(config.cls_classes/50))):
    ...
    if (i == int(math.ceil(config.cls_classes / 50)) - 1) and config.cls_classes % 50 != 0:
        print("select subset of data")
        index = config.cls_classes - i * 50
        s = s[0:config.nb_val_instances * index, :]
        l = l[0:config.nb_val_instances * index, :]
    x_valid.extend(s)
    y_valid.extend(l)
    del s
